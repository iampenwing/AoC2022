...
runMoves towers (move:moves) = runMoves (runMove towers move) moves

runMove :: [String] -> (Int, Int, Int) -> [String]
runMove towers (0, _, _) = towers
runMove towers (n, tower1, tower2) =
  let newTowers = moveCrate tower1 tower2 towers
  in runMove newTowers ((n-1), tower1, tower2)

popCrate :: Int -> [String] -> (Char, [String])
popCrate 1 (tower:towers) = ((head tower), ((drop 1 tower):towers))
popCrate n (tower:towers) = let (c, newTowers) = popCrate (n-1) towers
                            in (c, (tower:newTowers))

pushCrate :: Int -> Char -> [String] -> [String]
pushCrate 1 c (tower:towers) = ((c:tower):towers)
pushCrate n c (tower:towers) = (tower:(pushCrate (n - 1) c towers))


moveCrate :: Int -> Int -> [String] -> [String]
moveCrate x y towers = let (crate, newTowers) = popCrate x towers
                           in pushCrate y crate newTowers
